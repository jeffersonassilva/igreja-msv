...
<div class="flex flex-col relative bg-white border border-gray-200 p-4 md:px-6 rounded-md dark:bg-[#252c47] dark:border-[#252c47]">
    <div class="absolute left-[-3px] h-[30px] w-[3px]" style="background: {{ $escala->cor ?? '#ccc' }}"></div>
    <div class="text-gray-800 dark:text-[#d0d9e6] col-span-2">
        <div class="text-lg mb-1" style="color: {{ $escala->cor ?? '#6b7280' }}">{{ $escala->descricao }}</div>
        <div class="flex gap-1 text-xs font-thin">
            <div class="bg-gray-200 py-1 px-2 rounded-md dark:bg-[#343d61]">{{ \Carbon\Carbon::parse($escala->data)->format('d/m/Y') }}</div>
            <div class="bg-gray-200 py-1 px-2 rounded-md dark:bg-[#343d61]">{{ \Carbon\Carbon::parse($escala->data)->format('H:i') }}h</div>
            <div class="bg-gray-200 py-1 px-2 rounded-md dark:bg-[#343d61]">{{ \Carbon\Carbon::parse($escala->data)->dayName }}</div>
        </div>
    </div>
    ...
    @if($escala->justificativa)
        <div class="flex flex-col mt-4">
            ...
            <div class="text-gray-800 dark:text-[#d0d9e6] text-sm">{{ $escala->justificativa }}</div>
        </div>
    @endif
</div>
