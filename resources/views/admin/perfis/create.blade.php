        ...
        <form class="form-horizontal" role="form"
            ...
            <div class="flex flex-col mb-4 p-4 bg-white">
                ...
                <input type="text" name="descricao" id="descricao" maxlength="100"
                       class="border-gray-400 rounded-sm text-gray-700 @error('descricao') border border-red-500 @enderror"
                       value="{{ old('descricao') }}">
                @error('descricao')
                    ...
                    @foreach($permissoes as $permissao)
                        <div class="flex">
                            <div class="flex items-center h-5">
                                <input name="permissions[]" id="permissao-checkbox-{{ $permissao->id }}" type="checkbox" value="{{ $permissao->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300">
                            </div>
                            <div class="ml-2 text-sm">
                                <label for="permissao-checkbox-{{ $permissao->id }}" class="font-medium text-gray-900">
                                    {{ $permissao->descricao }}
                                </label>
