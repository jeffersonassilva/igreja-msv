...
<header>
    <table>
        <tr>
            ...
            <td style="border: 0; text-align: center; line-height: 1em;" class="title">
                {{ $title }}
                <br>
                ...
                <br>
                <span style="font-size: 14px; font-weight: normal;">
                    {{ $filtro->categoria ? 'Categoria: ' . \App\Services\CategoriaService::getDescricaoById($filtro->categoria) : null }}
                </span>
            </td>
            <td style="border: 0; vertical-align: top; text-align: right; font-size: 12px; color: #374151;">
...
<main>
    <div style="margin-bottom: 20px; text-align: center; font-size: 18px;">
        Valor Total: <span style="font-weight: bold;">{{ \App\Helpers\Strings::getMoedaFormatada($valorTotal, 'R$ ') }}</span>
    </div>
    ...
    <table style="margin-bottom: 40px; font-size: 14px;">
        ...
        <tbody>
        @foreach($data['itens'] as $key => $linha)
            <tr style="{{ $loop->even ? 'background: #eee;' : null  }}">
                <td style="width: 15%;">{{ \Carbon\Carbon::parse($linha['data'])->format('d/m/Y') }}</td>
                <td style="width: 35%;">{{ $linha['descricao'] }}</td>
                <td style="width: 15%;">{{ \App\Helpers\Strings::getMoedaFormatada($linha['valor'], 'R$ ') }}</td>
                <td style="width: 35%;">{{ \App\Services\CategoriaService::getDescricaoById($linha['categoria']) }}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
